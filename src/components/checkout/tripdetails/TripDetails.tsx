...
const TripDetails = ({ trip, height }: { trip: Trip, height: string }) => {
  ...
  const estimatedMiles = () => (
    (Math.abs(trip.until!.getTime() - trip.from!.getTime()) / (1000 * 60 * 60 * 24) + 1) * trip.car.distancePerDay
  )
    ...
    <Box sx={{ minHeight: { height } }} >
      ...
      <Grid container spacing={3}>
        <Grid item xs={12} md={6}>
          ...
          >
            <ImageSrc
              style={{ backgroundImage: trip!.car.images.length === 0 ? `url(${Placeholder})` : `url(data:${trip.car.images[0].contentType};base64,${trip.car.images[0].content.data})` }}
            />
          </Box>
        </Grid>
        <Grid item xs={12} md={6}>
